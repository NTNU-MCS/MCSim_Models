function [Zeta_a, Omega, Phase, Wavenum, Psi] = Wave_init(spectrum_type, hs, omega_peak, psi_mean,...
	gamma, spread, nfreq, ndir, energylim, freq_cutoff, dir_cutoff, rand_freq, rand_dir, rand_seed, plot_spectrum, plot_realization, disp_flag)
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%
...
%
% Calls:	GNC toolbox Wave spectra: mpierzon, jonswap, torset
%
...
if spectrum_type < 4
	...
	% Mean wave component energy, total sea state energy = m0 ~= (4*Hs)^2
	mean_energy = (4*hs)^2/(nfreq*ndir);
	
	% Open figure for gnc plotting
	figure
	...
	switch spectrum_type
		...
		case 1 % ITTC modified PM spectrum
			...
			[Omega_vec,S_vec,Tz] = mpierson(hs,omega_peak,omega_max,nfreq);
			
		case 2 % JONSWAP spectrum
			
			% Calculate gamma value from DNV formula if outside validity range
			if gamma < 1 | gamma > 7
				
				kk = 2*pi/omega_peak/sqrt(hs);
				...
				if disp_flag ~= 0
					...
					disp(sprintf(' Calculated gamma value         : %0.2f', gamma))
					
				end	
				
			end
			
			[Omega_vec,S_vec,Tz] = jonswap(hs,omega_peak,omega_max,gamma,nfreq);
			
		case 3 % Doubly Peaked spectrum
			
			[Omega_vec,S_vec] = torset(hs,omega_peak,omega_max,nfreq);
	...
	if Zeta_a(1) < 0
		...
		if plot_spectrum > 0
			...
			elseif ndir == 1
				...
				xlabel('\omega (rad/s)')
				ylabel('S [m^2s]')
				title(sprintf('Long-crested wave spectrum, direction %0.0f degrees',psi_mean*180/pi))
